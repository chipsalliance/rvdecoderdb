    ...
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            Self::Register { index, value } => {
                write!(f, "write register x{index} with {value:#018x}")
            }
            Self::MemoryWrite { address, value } => {
                write!(f, "write memory {address:#018x} with {value:#018x}")
            }
            Self::MemoryRead { address } => write!(f, "read memory {address:#018x}"),
        }
    }
}
